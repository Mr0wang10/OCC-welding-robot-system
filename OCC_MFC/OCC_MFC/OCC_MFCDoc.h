...
private:
    ...
    // 每个子模型的{面→边}映射
    std::vector<TopTools_IndexedDataMapOfShapeListOfShape> m_SolidFaceEdgeMaps;

    void BuildTopologyMaps();

    // ----- 步骤2: 初始焊缝提取 -----
    void ExtractInitialWelds();
    bool HasOppositeNormals(const TopoDS_Face& face1, const TopoDS_Face& face2);
    bool IsContainmentRelation(const TopoDS_Face& face1, const TopoDS_Face& face2);
    void GetFaceParamRange(const TopoDS_Face& face, double& uMin, double& uMax, double& vMin, double& vMax);
    void ExtractWeldFromContainment(const TopoDS_Face& smallFace, const TopoDS_Face& bigFace,
                                     int shapeIdx1, int shapeIdx2);
    void ExtractWeldFromIntersection(const TopoDS_Face& smallFace, const TopoDS_Face& bigFace,
                                      int shapeIdx1, int shapeIdx2);
    void BindFormingFaces(WeldSeam& seam, const TopoDS_Face& face1, const TopoDS_Face& face2,
                           int shapeIdx1, int shapeIdx2);
    ...
    void HandleDoubleSideTruncation(WeldSeam& seam, const TopoDS_Solid& cuttingSolid);
